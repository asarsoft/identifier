			...
			<div class="dropdown">
				...
				<div class="dropdown-menu" aria-labelledby="language_selection">
					@foreach($records['languages'] as $language)
						<a class="dropdown-item" href="{{ route('edit-feature', ['id' => $record->id, 'parameter' => $language->id]) }}"><span class="text-uppercase mr-2 text-primary">{{ $language->language_code }}</span> {{ $language->name }}
						</a>
					@endforeach
				</div>
			</div>
